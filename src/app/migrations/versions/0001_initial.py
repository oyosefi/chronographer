...
def upgrade() -> None:
    ...
    op.execute(
        sa.text("INSERT INTO users (id, display_name, timezone) VALUES (:id, :name, :timezone)").bindparams(
    ...
    op.create_table(
        ...
        sa.CheckConstraint("expire_after_seconds IS NULL OR expire_after_seconds > 0", name="ck_expiry"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "completion_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("completions.id"), nullable=False
        ),
